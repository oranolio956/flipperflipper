#!/bin/bash
# GitHub-based VPS Deployment for Stitch RAT
# One-liner installation from GitHub
# Usage: curl -sSL https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_REPO/main/github_deploy.sh | bash
...
GITHUB_BRANCH="cursor/setup-and-manage-vps-with-plesk-1813"  # Current branch with deployment code
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Stitch RAT GitHub Deployment         ${NC}"
echo -e "${BLUE}========================================${NC}"
echo -e "${YELLOW}Deploying from: https://github.com/$GITHUB_USER/$GITHUB_REPO${NC}"
...
if [[ $EUID -ne 0 ]]; then
   ...
   echo -e "${YELLOW}Run: sudo bash or switch to root user${NC}"
   exit 1
fi
...
if [[ "$ID" != "ubuntu" && "$ID" != "debian" ]]; then
    ...
    echo -e "${YELLOW}Continuing anyway, but some commands might fail...${NC}"
fi
...
apt install -y redis-server sqlite3
...
echo -e "${YELLOW}Step 4: Cloning repository from GitHub...${NC}"
# Clone the repository
cd /opt
git clone -b $GITHUB_BRANCH https://github.com/$GITHUB_USER/$GITHUB_REPO.git stitchrat-temp

# Move contents to proper directory
mv stitchrat-temp/* $APP_DIR/
mv stitchrat-temp/.* $APP_DIR/ 2>/dev/null || true  # Move hidden files, ignore errors
...
# Install cryptography first (needed for SSL)
...
if [ -f "$APP_DIR/requirements.txt" ]; then
    ...
    echo -e "${RED}requirements.txt not found! Installing basic dependencies...${NC}"
...
echo -e "${YELLOW}Step 11: Configuring fail2ban...${NC}"
cat > /etc/fail2ban/jail.d/stitchrat.conf << 'EOF'
[stitchrat]
enabled = true
port = 443,80
filter = stitchrat
...
cat > /etc/fail2ban/filter.d/stitchrat.conf << 'EOF'
[Definition]
failregex = ^<HOST> -.*"(GET|POST).*(login|admin).*" (4\d\d|5\d\d)
ignoreregex =
EOF
...
chown $APP_USER:$APP_USER $APP_DIR/update_from_github.sh
...
echo -e "Repo: ${GREEN}https://github.com/$GITHUB_USER/$GITHUB_REPO${NC}"
...
echo -e "Update from GitHub: ${GREEN}stitchrat-update${NC}"
...
echo -e "4. fail2ban is configured to protect against brute force"
echo -e "5. Firewall is configured with minimal required ports"
echo ""
...
echo ""
echo -e "${GREEN}🎉 Deployment successful! Access your RAT at: https://$VPS_IP${NC}"