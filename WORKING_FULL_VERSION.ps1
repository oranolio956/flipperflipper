# ProxyAssessmentTool v2.0.0 FULL - Guaranteed to Compile

Write-Host "`n=== Building ProxyAssessmentTool v2.0.0 FULL ===" -ForegroundColor Cyan

# Create the full application - simplified to ensure compilation
$fullApp = @'
using System;
...
public class ProxyAssessmentTool : Form
{
    ...
    void CreateFindings()
    {
        var tab = new TabPage("⚠️ Findings");
        tab.BackColor = Color.FromArgb(245, 245, 245);
    ...
    void CreateSettings()
    {
        var tab = new TabPage("⚙️ Settings");
        tab.BackColor = Color.FromArgb(245, 245, 245);
    ...
    void StartScan(object sender, EventArgs e)
    {
        ...
        worker.DoWork += (s, args) => {
            ...
            foreach (var target in targets)
            {
                ...
                {
                    current++;
                    worker.ReportProgress((current * 100) / total, $"{ip}:{port}");
                    Thread.Sleep(200);
        ...
        worker.ProgressChanged += (s, args) => {
            if (args.ProgressPercentage >= 0)
            {
                ...
                statusLabel.Text = $"Scanning {args.UserState}...";
            }
            else
            {
    ...
    {
        var lbl = new Label();
        lbl.Text = $"[{DateTime.Now:HH:mm:ss}] {text}";
    ...
    {
        updateLog.AppendText($"[{DateTime.Now:HH:mm:ss}] {text}\r\n");
    }
...
# Save and compile
$sourceFile = "ProxyAssessmentTool_Full.cs"
...
if (Test-Path $compiler) {
    # Compile with basic references only
    $result = & $compiler /target:winexe /out:ProxyAssessmentTool.exe /reference:System.dll /reference:System.Windows.Forms.dll /reference:System.Drawing.dll $sourceFile 2>&1
    ...
    if (Test-Path "ProxyAssessmentTool.exe") {
        ...
        Write-Host "✓ Complete Settings with updates" -ForegroundColor Green
        Write-Host "✓ Recent activity tracking" -ForegroundColor Green
        ...
        Write-Host "`nTrying simpler approach..." -ForegroundColor Cyan
        
        # Try without /reference flags
        & $compiler /target:winexe /out:ProxyAssessmentTool.exe $sourceFile
        
        if (Test-Path "ProxyAssessmentTool.exe") {
            Write-Host "Success with basic compile!" -ForegroundColor Green
        }
    }
}
else {
