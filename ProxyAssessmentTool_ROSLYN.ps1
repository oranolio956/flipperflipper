# ProxyAssessmentTool with Roslyn Compiler
param([string]$OutputPath = "ProxyAssessmentTool.exe")

$ErrorActionPreference = "Stop"
Write-Host "ProxyAssessmentTool Builder with Roslyn" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan

# Setup paths
$toolsDir = Join-Path $env:LOCALAPPDATA "ProxyAssessmentTool\Tools"
$roslynDir = Join-Path $toolsDir "Roslyn"
$cscPath = Join-Path $roslynDir "csc.exe"
$buildDir = Join-Path $env:TEMP "PAT_Build_$(Get-Random)"

# Check if Roslyn is installed
if (-not (Test-Path $cscPath)) {
    Write-Host "`nRoslyn compiler not found. Downloading..." -ForegroundColor Yellow
    
    # Create directories
    New-Item -ItemType Directory -Path $toolsDir -Force | Out-Null
    New-Item -ItemType Directory -Path $roslynDir -Force | Out-Null
    
    try {
        # Download Roslyn
        $roslynUrl = "https://www.nuget.org/api/v2/package/Microsoft.Net.Compilers.Toolset/4.8.0"
        $tempZip = Join-Path $env:TEMP "roslyn_temp.zip"
        
        Write-Host "Downloading Roslyn compiler..." -ForegroundColor Gray
        $webClient = New-Object System.Net.WebClient
        $webClient.DownloadFile($roslynUrl, $tempZip)
        
        # Extract
        Write-Host "Extracting compiler..." -ForegroundColor Gray
        Add-Type -AssemblyName System.IO.Compression.FileSystem
        $tempExtract = Join-Path $env:TEMP "roslyn_extract_$(Get-Random)"
        [System.IO.Compression.ZipFile]::ExtractToDirectory($tempZip, $tempExtract)
        
        # Copy compiler files
        $compilerSource = Join-Path $tempExtract "tools\bincore"
        if (-not (Test-Path $compilerSource)) {
            $compilerSource = Join-Path $tempExtract "tools"
        }
        
        Copy-Item -Path "$compilerSource\*" -Destination $roslynDir -Recurse -Force
        
        # Cleanup
        Remove-Item $tempZip -Force
        Remove-Item $tempExtract -Recurse -Force
        
        Write-Host "Roslyn compiler installed successfully!" -ForegroundColor Green
    }
    catch {
        Write-Host "Failed to download Roslyn. Falling back to system compiler..." -ForegroundColor Yellow
    ...
    Write-Host "Roslyn compiler found at: $cscPath" -ForegroundColor Green
}

Write-Host "`nBuilding ProxyAssessmentTool..." -ForegroundColor Cyan
...
namespace ProxyAssessmentTool
{
    ...
    public class MainForm : Form
    {
        ...
        private void OnExportClick(object sender, EventArgs e)
        {
            try
            {
                var filename = $"eligible_proxies_{DateTime.Now:yyyyMMdd_HHmmss}.txt";
...
del ""%~f0""";
        ...
        private void AddProxyCard(ProxyInfo info)
        {
            ...
            {
                Text = $"Tested: {DateTime.Now:HH:mm:ss}",
        ...
        {
            statusLabel.Text = $"Found: {totalFound} | Eligible: {eligibleCount} | Auto-Update: ON";
            scanningLabel.Text = $"Scanning internet for proxies... ({totalFound} found)";
...
'@
    ...
    if ($LASTEXITCODE -eq 0 -and (Test-Path $OutputPath)) {
        ...
        Write-Host "  * Modern async/await pattern" -ForegroundColor Gray
        Write-Host ""
        if ($cscPath -like "*Roslyn*") {
            Write-Host "Using: Roslyn C# Compiler" -ForegroundColor Yellow
        } else {
