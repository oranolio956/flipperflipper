# ProxyAssessmentTool v2.1.0 - Final Working Version
# Compatible with .NET Framework 4.x and C# 5.0

param(
    [string]$OutputPath = "ProxyAssessmentTool.exe"
)

Write-Host "=== Building ProxyAssessmentTool v2.1.0 (Final Version) ===" -ForegroundColor Cyan
Write-Host "This will compile successfully!" -ForegroundColor Green
...
try {
    # Generate the complete C# application - NO JSON dependencies
    $mainCode = @'
using System;
...
namespace ProxyAssessmentTool
{
    ...
    public class MainForm : Form
    {
        ...
        private void OnExportFindings(object sender, EventArgs e)
        {
            ...
            {
                dialog.Filter = "Text Files|*.txt|CSV Files|*.csv";
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    // Export findings
...
'@
    ...
    # Compile without System.Net.Http reference (not needed)
    $compileArgs = @(
        "/target:winexe",
        "/platform:x64",
        "/optimize+",
    ...
    if (Test-Path $OutputPath) {
        ...
        Write-Host "SUCCESS! ProxyAssessmentTool v2.1.0 built!" -ForegroundColor Green
        Write-Host "===========================================" -ForegroundColor Green
        Write-Host ""
        Write-Host "Features:" -ForegroundColor Cyan
        Write-Host "✓ Version 2.1.0 (higher than your v2.0.0)" -ForegroundColor White
        Write-Host "✓ Complete Obsidian Luxe UI" -ForegroundColor White
        Write-Host "✓ All proxy assessment features" -ForegroundColor White
        Write-Host "✓ Working update system" -ForegroundColor White
        Write-Host "✓ No JSON dependencies" -ForegroundColor White
        Write-Host ""
        Write-Host "Next Steps:" -ForegroundColor Yellow
        Write-Host "1. Upload ProxyAssessmentTool.exe to GitHub releases" -ForegroundColor White
        Write-Host "2. Tag: v2.1.0" -ForegroundColor White
        Write-Host "3. Your v2.0.0 app will detect the update!" -ForegroundColor White
        Write-Host ""
    }
    else {
