...
SectionEnd'''.format(nsis_Version[name],nsis_ProductName[name],nsis_CompanyName[name],
    ...
    nsis_client_app = run_command('"C:\\Program Files (x86)\\NSIS\\makensis.exe" "{}"'.format(nsis_script))
    if no_error(nsis_client_app):
        ...
        #st_print('[+] NSIS client_app complete')
    else:
        st_print('[!] Error creating NSIS client_app with {} configuration'.format(exe_name))
