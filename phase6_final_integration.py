...
'''
        ...
        print("\n  [Payload]")
        subprocess.run('python3 /tmp/payload_gen.py > /dev/null 2>&1', shell=True, capture_output=True)
    ...
    def cleanup(self):
        ...
        # Kill any stragglers
        subprocess.run("pkill -f 'python.*c2.py' 2>/dev/null", shell=True, capture_output=True)
        subprocess.run("pkill -f 'python.*patched_web' 2>/dev/null", shell=True, capture_output=True)
        subprocess.run("pkill -f 'python.*payload.py' 2>/dev/null", shell=True, capture_output=True)
